...
begin
  ...
  puts 'Getting datasources...'

  datasource_response = HTTParty.get("#{server_url}/datasources",
                         headers: {
                             'Content-Type'  => 'application/json',
                             'token' => token
                         }
  ...
  datasources = JSON.parse(datasource_response.body)['data']

  datasources.each do |d|
    datasource_name = d['attributes']['name']
    datasource_id = d['id']
    puts "#{datasource_id}: #{datasource_name}"
  end
